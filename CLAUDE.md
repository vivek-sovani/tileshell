...
- Last completed session: S11 — pin from app list (FR-5), **Phase 2 complete, tagged `v0.2`**. A 450 ms long-press (7 px cancel; `tapOrLongPress` raw pointer gesture, non-consuming so vertical scroll still wins on a drag) on an app row calls `AppListViewModel.pin` → `LayoutRepository.pinApp`, which appends a MEDIUM `TileEntity` at `MAX(position)+1` in the app's default colour (`TileColors.defaultIdFor`, deterministic per-package fold into the 14 palette ids; `iconKey=null`). De-dupe via `LayoutDao.appTileCount` (top-level app tiles by package). Outcome flows back on `pinned: SharedFlow<PinOutcome>`; the screen toasts "pinned <label>" / "already on start" and, on success, `onPinned` → `settleTo(0f)` returns to Start. Also folded in S10 (was half-done/uncommitted): live `BasicTextField` search wired to `filteredApps` (case-insensitive substring, `AppListFilter`, "no apps found" empty state) + a 4-col jump grid (tap a section header → overlay; present letters scroll to section, `#` is a real target, absent dim & dismiss). `TileColors`/`AppListFilter` unit-tested. Build green; not yet re-verified on emulator.
- Next session: S12 · Edit mode entry/exit + chrome (Phase 3)

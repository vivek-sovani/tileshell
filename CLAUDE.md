...
- Last completed session: S13 — drag to reorder (FR-3.2, Phase 3). In edit mode a grid-level `editDragGesture` (pointer positions grid-local) lifts the pressed tile after a 7 px move: it scales to 1.08 with a shadow + raised z and follows the finger, while the rest re-flow live. Reorder fires when the finger hovers the **edge** zone of another tile (`reorderTiles`, prototype splice-move with a `lastTarget` guard); the inner 22–78% (`inMergeZone`) is left for the S14 merge. Drop persists via `StartViewModel.reorder` → `LayoutRepository.reorderTiles` → `LayoutDao.applyOrder` (one transaction renumbering `position`). Auto-scroll: the gesture sets a −1/0/+1 dir from the finger's viewport-Y (content→viewport via status-bar inset + `scrollState.value`), a `LaunchedEffect` scrolls one step/frame until it leaves the edge zone or hits the bound. **`DenseTileGrid` was inverted** from a custom `Layout` to a `BoxWithConstraints`+sized `Box` whose children self-position via `Modifier.offset` — it hands each tile its `(slot, sizePx)` from the new shared **`GridGeometry`** (same unit/gap/side/top math), `key(p.id)` per tile so `animateIntOffsetAsState` reflow survives a re-pack; the dragged tile uses the live drag offset instead. Working order is a local `SnapshotStateList` reconciled (not reset) from the persisted flow → no post-drop snap-back flicker. `reorderTiles` unit-tested (`TileReorderTest`, 10 cases incl. direction-dependent landing). Build + tests green; not yet re-verified on emulator.
- Next session: S14 · Merge to folder (Phase 3)

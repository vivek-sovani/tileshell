...
- Last completed session: S5 — Room schema + default-layout seeder in `:core:data` (Room 2.6.1 via KSP). Entities `tiles` / `folders` / `folder_children` (FK CASCADE) / `app_cache`; nested `@Relation` (`TileWithFolder`→`FolderWithChildren`); `TileSize` TypeConverter; schema exported to `core/data/schemas` (v1), `TileShellDatabase.MIGRATIONS` empty array wired in. `LayoutRepository` exposes `Flow<List<TileModel>>` (sealed App/Folder) and `seedIfEmpty()`. Seeder: `DefaultLayout.DEFAULT_TILES` (ported from data.js) + `roleFor` (role id→intent/category/DefaultSms) + `AndroidRoleResolver` (resolves to the package's launcher activity) + pure `LayoutSeeder` (skips roleless/unmatched, dedups + drops empty folders, contiguous positions). 8 seeder JUnit tests. `TileSize` moved to `:core:data`; `:feature:start` now depends on it. Verified on emulator: seeds 14/20 tiles to real apps (clock→Deskclock, mail→Gmail, …), 6 unmatched skipped, social folder = 3 deduped children; Flow + nested relation query work.
- Next session: S6 · Real Start screen + launching
- Known issues: pre-Q (API 26–28) default-launcher prompt falls back to Settings ACTION_HOME_SETTINGS; isDefault uses RoleManager on Q+, resolveActivity heuristic below. `DebugAppListScreen` (S2) and `GridSpikeScreen` (S3) are temporary scaffolding still in the tree; both are superseded by the real Start screen in S6. MainActivity currently displays `GridSpikeScreen`. Wallpaper radial layers approximate the CSS ellipse radius with a circle (reads identically). `app_cache` table + DAO ops defined but not yet populated (wired up in S6/S9). `:core:design` keeps a separate private preview-only size enum (intentional — avoids a data-layer dep).

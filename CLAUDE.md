...
- Last completed session: S15 — resize, unpin, edit bar (FR-3.4/3.5, Phase 3). The corner controls and edit-bar buttons (inert chrome since S12) are now wired. **Resize**: tapping the selected tile's bottom-right corner cycles size via `TileSize.next()` (small→medium→wide→large→small) → `StartViewModel.resize` → `LayoutRepository.cycleTileSize` → `LayoutDao.updateTileSize`; the re-pack animates through the existing S13 `animateIntOffsetAsState` reflow. **Unpin**: top-left corner → `StartViewModel.unpin` → `LayoutDao.removeTile` (deletes the tile + its `folders` row, cascading children); optimistically removed from the working order. Both corner taps are handled inside `editDragGesture` via 30 dp hot-zones (not child buttons) and consumed, so the new `isConsumed` guard in `emptySpaceExit` keeps them from also exiting edit. **Edit bar**: add → `exitEdit` + `settleTo(1f)` + "long-press an app to pin" toast; personalize → `PersonalizeStubSheet` (scrim + placeholder panel, full sheet later with `:feature:personalize`); done → `exitEdit`. The drag gesture is re-keyed on `byId` so a mid-session resize/unpin refreshes its hit-test sizes. Note: Room rejects a `TileSize` converter on a `@Query` param/scalar return — size is read via `tilesOnce()` and bound as its `.name` string. `TileSize.next()` unit-tested (`TileSizeCycleTest`, 2 cases). Build + tests green; not yet re-verified on emulator.
- Next session: S16 · Folder overlay + rename (Phase 3)

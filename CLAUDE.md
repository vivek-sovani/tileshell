...
- Last completed session: S9 — real alphabetical app list (FR-5) in `:feature:applist`. `AppListScreen` + `AppListViewModel` (AndroidViewModel, `AppCatalogRepository.apps.stateIn`): a `LazyColumn` with lowercase accent letter section headers (emitted when `AppEntry.letter` changes) and rows = 44dp accent square backing the **real app icon** (loaded via `PackageManager.getActivityIcon` off-main in `produceState`; generic "app" glyph fallback) + name; tap → `AppLauncher.launch`. Shared `AppLauncher` extracted to `:core:data` (StartScreen now uses it too). Visual-only search bar at top (filtering is S10). `:feature:start` depends on `:feature:applist`; the pager's App-list page now renders `AppListScreen` (placeholder removed). Verified on emulator: real apps listed A–Z with icons + headers, tapping Chrome launched it, uninstalling Chrome removed its row live.
- Next session: S10 · Search + jump grid

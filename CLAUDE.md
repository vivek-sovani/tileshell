...
- Last completed session: S7 — tilt feedback + Home behaviour. `Modifier.tiltOnPress()` in `:core:design` (FR-1.2): non-consuming `awaitEachGesture` observer drives `graphicsLayer` rotationX/Y toward the contact point (±12°) + 0.95 dip while pressed, springs flat on release; applied on each tile (taps still fire). `StartViewModel.goHome()` emits a `homeRequests` SharedFlow; `StartScreen` collects it and `animateScrollTo(0)`. MainActivity is now the activity-scoped `StartViewModel` owner (`by viewModels()`, passed into `StartScreen`); `onNewIntent` (singleTask HOME re-delivery) → `goHome()`; an always-enabled `OnBackPressedCallback` makes Back on Start a no-op (will close edit/overlays in S12/S16). Uninstall auto-removal (FR-5): `LayoutDao.removeApp` (delete tiles + folder_children by package, then empty folder tiles/meta) wired to a `LauncherApps.Callback` in the ViewModel (onPackageRemoved/Unavailable). Verified on emulator: corner-press tilts the tile in 3D and springs back; Home scrolls to top; Back stays on Start; uninstalling Chrome removes the browser tile live and the packer re-flows.
- Next session: S8 · Start ⇄ App list pager

...
- Last completed session: S14 — merge to folder (FR-3.3, Phase 3). The S13 drag gesture's centre branch is now live: hovering the inner 22–78% (`inMergeZone`) of another tile marks it a merge target (full-opacity + 3 px inset `Modifier.border` highlight, prototype `.merge-target`); releasing there merges instead of reordering and toasts "grouped". Rules live in pure `computeMerge(drag, target)` (`:core:data/TileMerge.kt`): target folder → drag's apps appended, deduped by component (`packageName/activityName`); target app → a new folder **reusing the target tile's id** (DECISIONS S5 convention) with target-then-drag apps, size = target's (small→medium), target's colour, name "folder". Folder→folder is a child union (drag folder + its `folders` row cascade-deleted). Persisted by `StartViewModel.merge` → `LayoutRepository.mergeTiles` → `LayoutDao.applyMerge`: one `@Transaction` that rewrites the target as a folder tile, replaces its `folder_children`, drops the dragged tile, and **renumbers the surviving working order** so any incidental reorder on the way to the target is saved too. Folder face (2×2 mini-grid) unchanged since S6. `computeMerge` unit-tested (`TileMergeTest`, 8 cases incl. folder-into-folder union + dedup + small-promotion). Build + tests green; not yet re-verified on emulator.
- Next session: S15 · Resize, unpin, edit bar (Phase 3)

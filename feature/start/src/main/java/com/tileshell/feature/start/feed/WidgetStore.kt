...
/**
 * One-time migration for an install upgrading from before the synthetic
 * "gadget" cards (`CustomCardKind`) were removed: those cards were negative
 * ids below the three builtin sentinels (see `nextCustomWidgetId`'s old
 * doc comment), which — with `HostedWidget.customKind`/`customConfig` now
 * gone — would otherwise sit in the list looking like a real, never-
 * resolving hosted widget. Rather than let each one silently self-heal via
 * [WidgetView]'s own ~15s "provider never resolved" grace period (safe, but
 * a visible several-second gap/flicker in its row first), this strips them
 * immediately: any [HostedWidget] whose id is negative and NOT one of the
 * three known builtin sentinels. A dangling single-member stack left behind
...
internal fun stripStaleNegativeIds(current: List<HostedWidget>): List<HostedWidget> {
    ...
    val loneStackIds = kept.groupBy { it.stackId }.filter { (id, members) -> id != null && members.size == 1 }.keys
    return if (loneStackIds.isEmpty()) kept else kept.map { if (it.stackId in loneStackIds) it.copy(stackId = null) else it }
}
...
/**
 * One widget per line as `id,heightDp,widthDp,halfWidth,stackId`; tolerant
 * (bad lines dropped, missing fields all fall back to defaults) — the
 * trailing `halfWidth`/`stackId` fields are optional so lines written before
 * either existed still decode fine. An un-stacked widget writes `stackId` as
 * an empty field rather than a sentinel like `-1`, so it decodes identically
 * to an older file that never carried the column at all. A save file written
 * while this format still had two further `customKind,customConfig` columns
 * (the now-removed synthetic "gadget" cards) still decodes cleanly — any
 * columns past the 5th are simply ignored.
 */
object WidgetCodec {
...
class WidgetStore(private val store: DataStore<WidgetData>) {
    ...
    /**
     * One-time housekeeping run at startup — see [seedMissingBuiltinWidgets]
     * and [stripStaleNegativeIds] for the actual logic.
     */
    suspend fun seedBuiltinsIfAbsent() {
        store.updateData { data -> data.copy(widgets = seedMissingBuiltinWidgets(stripStaleNegativeIds(data.widgets))) }
    }
